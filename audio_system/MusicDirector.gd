...
func _process(_delta: float) -> void:
	...
	match state:
		STATE_TRAVELING_FORWARD:
			var target := cue_catalog.get_candidate(current_track_id, pending_cue_id)
			if not target.is_empty() and current_playback_position + POSITION_EPSILON_SECONDS >= float(target.get("start_seconds", 0.0)):
		...
		STATE_REWIND_PENDING:
			if _rewind_boundary_end >= 0.0 and (current_playback_position + POSITION_EPSILON_SECONDS >= _rewind_boundary_end or current_playback_position + 0.25 < _last_playback_position):
...
func audition_candidate(track_id: String, candidate_id: String, lead_in_bars := 0) -> bool:
	...
	current_cue_id = candidate_id
	pending_cue_id = ""
	audio_player.play(start_seconds)
	_set_state(STATE_HOLDING_CUE if lead_in_bars == 0 else STATE_PLAYING_SOURCE)
	return true
...
func request_cue(cue_id: String) -> bool:
	...
	if target_start > current_start + POSITION_EPSILON_SECONDS:
		...
		authored_transition_started.emit(from_cue, cue_id)
		return true
	if current.is_empty() or state != STATE_HOLDING_CUE:
		return _jump_and_hold(cue_id)
	runtime_stream.loop_mode = AudioStreamWAV.LOOP_DISABLED
	pending_cue_id = cue_id
	_rewind_boundary_end = float(current.get("end_seconds", current_playback_position))
...
func release_to_next_cue() -> bool:
	if current_track_id.is_empty():
		return _fail("No track is active")
	var approved := cue_catalog.get_approved_cues(current_track_id)
	var current := cue_catalog.get_candidate(current_track_id, current_cue_id)
	var current_bar := int(current.get("bar_start", -1))
	for cue in approved:
		if int(cue.get("bar_start", -1)) > current_bar:
...
func _jump_and_hold(cue_id: String) -> bool:
	...
	pending_cue_id = ""
	_rewind_boundary_end = -1.0
	audio_player.play(float(cue.get("start_seconds", 0.0)))
	current_playback_position = float(cue.get("start_seconds", 0.0))
	_last_playback_position = current_playback_position
	_set_state(STATE_HOLDING_CUE)
...
func _read_position() -> float:
	...
	return fposmod(value, stream_length_seconds) if stream_length_seconds > 0.0 else maxf(0.0, value)
