...
func score_new_meld(cards: Array[CardData], meld_type: String, phase: int) -> ScoringContext:
...
func score_extension(
	...
	_apply_modifiers(context)
	context.theoretical_score = _calculate_theoretical(context)
	context.final_points = maxi(context.theoretical_score - old_meld_score, 0)
	context_scored.emit(context)
	return context
